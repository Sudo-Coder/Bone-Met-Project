...
TAB = os.path.join(ROOT, "analysis", "pca_comparator", "outputs", "tables")
...
for tag, cfg in OBJS.items():
    ...
    df["patient_id"] = a.obs["Sample"].map(ppid).values
    df["cancer_type"] = tag
    df["Sample"] = a.obs["Sample"].values
    per.append(df)
pc = pd.concat(per)
pc.to_parquet(os.path.join(TAB, "pca_mirror_module_scores_percell.parquet"))
...
def tvb(dfc, m):
    ...
    try:
        ...
        ci = mm.conf_int().loc["C(condition, Treatment('Benign'))[T.Tumor]"]
        e, se, p = mm.params[1], (ci[1]-ci[0])/3.92, mm.pvalues[1]
    return e, e-1.96*se, e+1.96*se, p, sd
def gradient(dfc, m):
    order = {"Benign": 0, "Distal": 1, "Involved": 2, "Tumor": 3}
    d = dfc[dfc.condition.isin(order)].dropna(subset = [m]).copy()
    d["ord"] = d.condition.map(order)
    if len(d)<10: return (np.nan, np.nan, np.nan)
    fit = smf.ols(f"{m} ~ ord", d).fit(cov_type = "cluster", cov_kwds = {"groups": d["patient_id"]})
    ci = fit.conf_int().loc["ord"]
    return fit.params["ord"], fit.pvalues["ord"], (ci[0], ci[1])
...
for m in MODULES:
    rcc = tvb(pb[pb.cancer_type=="RCC"], m)
    pca = tvb(pb[pb.cancer_type=="prostate"], m)
    gr = gradient(pb[pb.cancer_type=="prostate"], m)

    dd = pb[pb.condition.isin(["Tumor", "Benign"])].dropna(subset = [m]).copy()
    dd["condition"] = pd.Categorical(dd["condition"], ["Benign", "Tumor"])
    dd["cancer_type"] = pd.Categorical(dd["cancer_type"], ["prostate", "RCC"])
    try:
        mm = smf.mixedlm(INT_form.format(m = m), dd, groups = dd["patient_id"]).fit(reml = False, method = "lbfgs")
        ie, ip = mm.params[INT_coef], mm.pvalues[INT_coef]
    except Exception:
        mm = smf.ols(INT_form.format(m = m), dd).fit(cov_type = "cluster", cov_kwds = {"groups": dd["patient_id"]})
        ie, ip = mm.params[INT_coef], mm.pvalues[INT_coef]
...
L = pd.DataFrame(loc)
L.to_csv(os.path.join(TAB, "pca_within_tumor_localization.csv"), index = False)
