...
RCC_TAB = os.path.join(ROOT, "analysis", "rcc_reinterpretation", "outputs", "tables")
rcc3 = pd.read_csv(os.path.join(RCC_TAB, "phase3_adjusted_associations.csv"))
...
for m in MODS:
    ...
    rcc_cyt = imm(rcc3, m, "cytotoxicity")
    pca_cyt = imm(pca3, m, "cytotoxicity")
    rcc_ex = imm(rcc3, m, "CD8_exhaustion")
    pca_ex = imm(pca3, m, "CD8_exhaustion")
...
def verdict(m, c, rc, pc):
    ...
    return "ambiguous"
C = pd.DataFrame(rows)
C["verdict"] = [verdict(m, None, None, None) for m in C.module]
