...
plt.tight_layout()
plt.savefig(os.path.join(out, 'nomogram.png'), dpi = 200)
plt.close()
...
plt.ylabel('observed (KM)')
plt.legend()
plt.title('Calibration')
plt.tight_layout()
plt.savefig(os.path.join(out, 'calibration.png'), dpi = 200)
plt.close()

t = 1825
p_death = 1 - cox.predict_survival_function(mv, times = [t]).T[t].values
time = mv['OS.time'].values
ev = mv['OS'].values
n = len(mv)
...
plt.ylim(-0.05, max(nb_model) + 0.02)
plt.xlabel('threshold probability')
plt.ylabel('net benefit')
plt.legend()
plt.title('Decision curve analysis (5-yr OS)')
plt.tight_layout()
plt.savefig(os.path.join(out, 'dca.png'), dpi = 200)
plt.close()
...
isc = a.obsm['score_aucell']
isc['bcr'] = [s[:15] for s in kx.index]
rg = df[['bcr', 'risk']].copy()
...
rows = []
for k in imm:
    hi = isc[isc.grp == 'high'][k]
    lo = isc[isc.grp == 'low'][k]
...
plt.tight_layout()
plt.savefig(os.path.join(out, 'riskgroup_immune.png'), dpi = 200)
plt.close()
