#!/usr/bin/env python
"""15_axis_support.py — Phase 2 synthesis: which pre-specified axes are supported / ambiguous / unsupported.
Integrates three independent lines (all "predicted signaling axes" unless spatial/protein/perturbation):
  (A) LR expression tumor-specificity  (12_lr_expression -> axis_lr_expression.csv)
  (B) CellChat communication prob to TAM receivers (13 -> cellchat_prespecified_axes_tumor.csv)
  (C) NicheNet ligand->CLEC_LAM-target activity rank (14 -> nichenet_prespecified_ranks.csv)
Verdict per axis: SUPPORTED (>=2 lines agree + tumor-relevant), AMBIGUOUS (mixed), UNSUPPORTED/NOT-TUMOR-GAINED.
Robust to missing CellChat/NicheNet outputs (reports which lines were available).
Run: envs/rcc_reinterp_venv/bin/python
"""
...
# axis -> ligand + cellchat pathway mapping (CellChat v2 pathway names; APOE = "ApoE")
...
def verdict(x):
    # A = ligand tumor-elevated (tumor-specificity); B = CellChat axis to TAM present; C = NicheNet top20
    A=x.get("A_tumor_elevated"); B=x.get("B_cellchat"); C=x.get("C_top20")
    delta=x.get("lig_delta_TvsB",0) or 0; lp=x.get("lig_p",1)
    present = (B is True) or (C is True)          # axis operates in the tumor niche
    ...
    if A is True:              return "AMBIGUOUS"           # tumor-elevated ligand, weak network
    if benign_biased:          return "NOT_TUMOR_GAINED"    # ligand higher in benign
    if present:                return "PRESENT_NOT_TUMOR_GAINED"  # in niche but ligand not tumor-induced
    return "UNSUPPORTED"
