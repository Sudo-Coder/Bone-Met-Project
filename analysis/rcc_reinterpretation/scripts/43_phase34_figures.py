...
ax.set_yticks(y)
ax.set_yticklabels([f"{m}  (p={p:.3f})" for m, p in zip(d.module, d.p)], fontsize = 8)
ax.set_xlabel("TCGA-KIRC OS  HR per SD (adjusted: panTAM+Obradovic+immune)")
ax.set_title("Phase 4 — TCGA-KIRC OS (continuous Cox, adjusted)", fontsize = 10)
plt.tight_layout()
plt.savefig(os.path.join(FIG, "figP34_1_tcga_os_forest.png"), dpi = 300)
plt.close()
...
ax.set_xticks(x)
ax.set_xticklabels(outs, fontsize = 8, rotation = 20)
ax.set_ylabel("adjusted std. coef")
ax.legend(fontsize = 7)
ax.set_title("Phase 3 — immune-evasion contrast (complement vs generic/other TAM)", fontsize = 10)
plt.tight_layout()
plt.savefig(os.path.join(FIG, "figP34_2_immune_evasion_contrast.png"), dpi = 300)
plt.close()
