...
for i, (r, title) in enumerate([(en_sig, '25-gene signature'), (en_deg, 'TAM-vs-monocyte DEGs')]):
    ...
    ax[i].set_xlabel('-log10 adjusted p')
    ax[i].set_ylabel('')
...
plt.title('CPTAC ccRCC OS by risk (logrank p=' + format(lr_ext.p_value, '.3f') + ')')
plt.xlabel('days')
plt.tight_layout()
plt.savefig(os.path.join(out, 'km_validation_external.png'), dpi = 200)
plt.close()
...
plt.title('CPTAC ccRCC time-dependent AUC')
plt.tight_layout()
plt.savefig(os.path.join(out, 'roc_validation_external.png'), dpi = 200)
plt.close()
...
rows = []
for mt in metrics:
    hi = tide[tide.grp=='high'][mt]
    lo = tide[tide.grp=='low'][mt]
...
plt.tight_layout()
plt.savefig(os.path.join(out, 'riskgroup_tide.png'), dpi = 200)
plt.close()
...
if os.path.exists(pred_path):
    ...
    for a in agents:
        ...
        hi = v.loc[gg[gg=='high'].index]
        lo = v.loc[gg[gg=='low'].index]
        u, p = mannwhitneyu(hi, lo)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out, 'riskgroup_drug.png'), dpi = 200)
    plt.close()
