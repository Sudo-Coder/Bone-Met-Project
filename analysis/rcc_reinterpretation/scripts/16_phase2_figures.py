...
FIG = os.path.join(ROOT, "analysis/rcc_reinterpretation/outputs/figures")
...
a = ax.copy()
a["short"] = a["axis"].str.replace("complement_", "C").str.replace("_TREM2", "→TREM2").str.replace("_MERTK", "→MERTK").str.replace("_TGFBR", "→TGFBR")
y = np.arange(len(a))[::-1]
axs[0].barh(y, a["lig_delta_TvsB"], color = ["#2E7D32" if v else "#B0553F" for v in a["A_tumor_elevated"]])
axs[0].axvline(0, color = "k", lw = .6)
axs[0].set_yticks(y)
axs[0].set_yticklabels(a["short"], fontsize = 8)
axs[0].set_title("(A) ligand Δ tumor−benign\n(green=tumor-elevated,p<0.05)", fontsize = 8)
axs[0].set_xlabel("Δ log-norm")
axs[1].barh(y, a["B_cellchat_maxprob"].fillna(0), color = "#4C72B0")
axs[1].set_yticks(y)
axs[1].set_yticklabels([])
axs[1].set_title("(B) CellChat max prob\nsender→TAM", fontsize = 8)
axs[1].set_xlabel("comm. prob")
...
axs[2].invert_xaxis()
axs[2].set_title("(C) NicheNet rank\n(of 116; lower=better)", fontsize = 8)
...
plt.savefig(os.path.join(FIG, "figP2_1_axis_support.png"), dpi = 300)
plt.close()
...
axd.set_yticks(yy)
axd.set_yticklabels(cl["pair"], fontsize = 7.5)
axd.set_xlabel("communication probability")
axd.set_title("CellChat: top predicted axes → TAM_CLEC_LAM (tumor niche)", fontsize = 10)
plt.colorbar(sc, label = "prob")
plt.tight_layout()
plt.savefig(os.path.join(FIG, "figP2_2_cellchat_clec_lam.png"), dpi = 300)
plt.close()

nn2 = nn.sort_values("rank")
fig, axn = plt.subplots(figsize = (6, 3.4))
axn.barh(np.arange(len(nn2))[::-1], nn2["aupr_corrected"],
         color = ["#2E7D32" if r<=20 else "#7B8794" for r in nn2["rank"]])
axn.set_yticks(np.arange(len(nn2))[::-1])
axn.set_yticklabels([f"{t} (r{int(r)})" for t, r in zip(nn2.test_ligand, nn2["rank"])], fontsize = 8)
axn.set_xlabel("NicheNet aupr_corrected")
axn.set_title("NicheNet ligand→CLEC_LAM-program activity\n(green=top20 of 116)", fontsize = 9)
plt.tight_layout()
plt.savefig(os.path.join(FIG, "figP2_3_nichenet_ranks.png"), dpi = 300)
plt.close()

fig, axl = plt.subplots(figsize = (7, 3.6))
x = np.arange(len(lr))
w = 0.38
...
axl.set_xticks(x)
axl.set_xticklabels(lr["ligand"], rotation = 45, ha = "right", fontsize = 8)
axl.set_ylabel("mean log-norm (patient)")
axl.legend(fontsize = 8)
axl.set_title("Pre-specified ligand expression in senders (tumor vs benign)", fontsize = 9)
plt.tight_layout()
plt.savefig(os.path.join(FIG, "figP2_4_ligand_tumor_vs_benign.png"), dpi = 300)
plt.close()
