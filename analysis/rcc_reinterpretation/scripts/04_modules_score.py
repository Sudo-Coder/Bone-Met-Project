...
for tag,cfg in OBJS.items():
    ...
    df["cancer_type"]=tag; df["dataset"]="rcc_bm" if tag=="RCC" else "prostate_kfoury"; df["Sample"]=a.obs["Sample"].values
    per.append(df)
pc=pd.concat(per); pc.to_parquet(os.path.join(TAB,"module_scores_percell.parquet"))
...
pb_all=pseudobulk(pc); pb_all=pb_all[pb_all.n_cells>=MIN]
...
def interaction(pb,m):
    ...
    try:
        ...
        ci=mm.conf_int().loc[coef]; est,se,p=mm.params[coef],(ci[1]-ci[0])/3.92,mm.pvalues[coef]; meth="ols_cluster"
...
for m in MODULES:
    r=res[(res.module==m)&(res.unit=='all_myeloid')].iloc[0]; print(f"  {m:22s} SD={r.score_SD:.4f}")
