...
for c in ['ATF3_NFkB', 'complement_C1Q']:
    ...
    s['PFI.time'] = pd.to_numeric(s['PFI.time'])
    s['PFI'] = pd.to_numeric(s['PFI']).astype(int)
    ...
    plt.title('TCGA-PRAD PFI by ' + c + ' (logrank p=' + format(lr.p_value, '.2e') + ')')
    plt.xlabel('days')
    plt.tight_layout()
    plt.savefig(os.path.join(out, 'pca_km_prad_' + c + '.png'), dpi = 200)
    plt.close()
